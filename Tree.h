...
template<class Key, class Value>
int efficientTreeToPointersArray(Node<Key,Value>** nodesArray, Node<Key,Value>* firstNode, Key* lastKey) //O(r)
{
    if(firstNode == NULL)
    {
        return -1;
...
Tree<Key,Value>* mergeTrees(Tree<Key,Value>& t1, Tree<Key,Value>& t2)
{
    ...
    mergeSort(mergedArray, t1.getSize(),t2.getSize(), t1_array, t2_array);
    delete[] t1_array;
    delete[] t2_array;
